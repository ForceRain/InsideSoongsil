...
struct hwa_too *play_hwa_too(int,struct hwa_too_gamer *, struct hwa_too_pan *,int *);
	...
	{	
		...
		if(samecheck==1)
		{
			...
			if(a==2)
			{
				...
				{
					samecheck=same_check_hwa_too(pan->eat, pae3.num, pindex);
					//뒷패를 선택하는 경우
					if(samecheck==1)
					{
						printf("어떤것을 드시겠습니까?");
...

		{
			...
			if(samecheck==1)
			{
				printf("앞에 있는 것을 선택하려면 1, 뒤에 있는 것을 선택하려면 2를 입력하세요.\n");
				scanf("%d",&a);
				if(a==2)
				{
					plus_pae=location_copy_hwa_too(pan->eat,temp_count+1);
...
int main(void) {
	...
	while (1){											//전체적인 while문
		...
		while (1) {											//한판당의 while문
			...
			{
				player[turn]->pae_in_hand=play_hwa_too(atoi(system_command),player[turn],pan,pturn); 
				if (hurry_check==0)
			...
			{
				stealing_hwa_too(player,turn,2);
				tot--;
			}
			//
			else
			...
			if ((player[turn]->point>=3) || (player[turn]->go_plus>=1))
			{
				if (turn==0)

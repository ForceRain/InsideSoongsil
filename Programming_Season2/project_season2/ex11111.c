...
struct hwa_too *play_hwa_too(int,struct hwa_too_gamer *, struct hwa_too_pan *,int *,struct hwa_too_gamer *[],int []);
...
struct hwa_too *play_hwa_too(int count,struct hwa_too_gamer *player,struct hwa_too_pan *pan, int *pturn,struct hwa_too_gamer *p[],int noo_ga_ssam[])
{
...
int main(void) {
	...
	int wow,i=0,j=0,command,turn=0,tot=0,sort=0,temp_turn=0,nine_check=0,hurry_check=0,before_turn=0;
	int *pturn=&turn;
	...
	while (1){											//전체적인 while문
		...
		for (i=0;i<3;i++)
		{
			for (j=0;j<4;j++)
			{
				noo_ga_ssam[i][j]=0;					//누가 쌌는지 확인하는 배열 초기화
			}
		}
		tot=0;											//총 횟수 초기화자
		hurry_check=0;
		...
		{																									//한판당의 while문(일반적인 한 게임)
			...
			}																		//화투를 이용한 점수확인 함수count_checking_hwa_too
			sort=0;
			for (i=0;i<3;i++)
			{
				while(1)
...
#endif
			if (((player[before_turn]->point>=3) || (player[before_turn]->go_plus>=1)) && before_turn>=0)
			{
				while(1)
					{
						if (before_turn==0)
			...
			{
				player[turn]->pae_in_hand=play_hwa_too(atoi(system_command),player[turn],pan,pturn,player,noo_ga_ssam[turn]); 
				if (hurry_check==0)
			...
			if (turn==-1)
				turn=0;											//turn이 음수인 경우에는 help,입력을 잘못 했을 경우,저장,불러오기,잔고보기이다.
			before_turn=turn;
			tot++;
			turn++;
			if(turn==3)

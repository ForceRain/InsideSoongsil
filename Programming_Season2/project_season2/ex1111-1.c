...
struct status_hwa_too{
	unsigned shake:1, chong_tong:1, go:3, pi_bak:1, go_bak:1, mung_tung:1, na_ga_ri:1;
};
...
int result_hwa_too(struct hwa_too_gamer *player[])
{
	...
	for (i=0;i<3;i++)
	{
		if ((max<player[0]->point) && (player[i]->go_plus>0) && (player[i]->player_status.na_ga_ri==0))
			printf("승자는 A입니다!!!!\n");
		else if ((max<player[1]->point) && (player[i]->go_plus>0) && (player[i]->player_status.na_ga_ri==0))
			printf("승자는 B입니다!!!!\n");
		else if ((player[2]->go_plus>0) && (player[i]->player_status.na_ga_ri==0))
			printf("승자는 C입니다!!!!\n");
		else if (i==0)
			printf("이번판은 나가리입니다.\n");
	}
...
void na_ga_ri(int ggo, int total,struct hwa_too_gamer *player[],int turn)
{
	//나가리 확인 시작
	if (ggo<=0 && total==20)
	{
		//고 없이 끝까지 점수 안나는 함수
		//나가리 변수에 어떤 짓거리를 해줌 X2
		printf("나가리!! 다음판 두배!!\n");
		sleep(2);
		player[turn]->player_status.na_ga_ri=1;
	}
	else if (ggo>0 && total==20)										//고는 했으나 총 20번이 되도록 점수가 안남
	{
		//고 했는데 점수 끝까지 안나는 함수
		//나가리 변수에 어떤 짓거리를 해줌 X2
		printf("나가리!! 다음판 두배!!\n");
...
int check_na_ga_ri(struct hwa_too_gamer *player[])
{
	...
	for (i=0;i<3;i++)
	{
		sum+=player[i]->player_status.na_ga_ri;
	}
	if (sum>0)
		return 2;
	else
...
int main(void) {
	...
	while (1){																								//전체적인 while문
		...
		{
			notice_na_ga_ri*=check_na_ga_ri(player);				//전판이 나가리였을경우
		}
			else
			notice_na_ga_ri=check_na_ga_ri(player);					//전판이 나가리가 아니었을 경우
#if DEBUG
		printf("현재는 나가리가 적용되어 X%d배 입니다.\n",notice_na_ga_ri);
#endif
		before_turn=turn;
		for (i=0;i<3;i++)
		{
		...
		{																								//한판당의 while문
				...
				while(1)
					{
						for (i=0;i<3;i++)
						{
							...
							before_go_check[i]=player[i]->point;
						}
			...
			for (i=0;i<3;i++)
			{
				na_ga_ri(ggo,tot,player,i);
			}
#if DEBUG
			printf("A나가리가 되었느냐? : %d\n",player[0]->player_status.na_ga_ri);
			printf("B나가리가 되었느냐? : %d\n",player[1]->player_status.na_ga_ri);
			printf("C나가리가 되었느냐? : %d\n",player[2]->player_status.na_ga_ri);
#endif
			//나가리 확인 끝
		/*	if (turn==-1)

...
struct status_pan_hwa_too{
	unsigned shake:1, poop:1, eat_all:1, chong_tong:1, na_ga_ri:1, pi_bak:1, go_bak:1, mung_tung:1;
};
...
struct hwa_too *play_hwa_too(int,struct hwa_too_gamer *, struct hwa_too_pan *,int *,struct hwa_too_gamer *[]);
...
struct hwa_too *play_hwa_too(int count,struct hwa_too_gamer *player,struct hwa_too_pan *pan, int *pturn,struct hwa_too_gamer *p[])
{
	int i,j,input=0;
	int wow=0;
	int eat_check=0;
	...
	if (pae[0].num!=pae[2].num)					//내는것과 뒤집은것이 다른 경우
	{
			...
			//0=손에서 냄, 1=깔린것에서 손에서 내는것과 같은것, 2=뒤집는것, 3=깔린것에서 뒤집은 것과 같은것
		{
			...
			else if (num_check[pae[i].num]==2)
			{
				eat_check=compare_hwa_too(pae[i].num,pan->eat,1);
				printf("앞의 것을 선택하려면 1을, 뒤의 것을 선택하려면 2를 입력하세요.\n");
				scanf("%d",&input);
				if (input==1)
				{
					pae[i+1]=location_copy_hwa_too(pan->eat,eat_check);
...
void stealing_hwa_too(struct hwa_too_gamer *p[],int turn,int point)
{
	...
	int proce[3][3]={{0,1,2},{1,2,0},{2,0,1}};
	int use[3]={0};
	if (turn==0)
	{
		for (i=0;i<3;i++)
...
int find_mungtong_hwa_too(struct hwa_too *temp)
{
	int count=0;
	for (;temp;temp=temp->next)
	{
...
struct hwa_too *clear_bomb(struct hwa_too_gamer *p)
{
	int i=0;
	struct hwa_too *temp;
	temp=p->pae_in_hand;
	...
	{
		i++;
		if ((strcmp(temp->name,"폭탄"))==0)
		{
			break;							//폭탄의 위치를 찾음
		}
	}
	p->pae_in_hand=disconnect_hwa_too(p->pae_in_hand,i);
	return p->pae_in_hand;
}
int main(void) {
	...
	while (1){											//전체적인 while문
		...
		while (1) {											//한판당의 while문
			...
			for (i=0;i<3;i++)
			{
				...
				player[i]->go_plus=player[i]->point;
			}
			sort=0;
			for (i=0;i<3;i++)
			{
					...
					for(i=0;i<3;i++)
						{
							...
							{
								if(i==0)
									player[i]->money+=money_[1]+money_[2];
								if(i==1)
									player[i]->money+=money_[0]+money_[2];
								if(i==2)
									player[i]->money+=money_[0]+money_[1];
							}
						}
//돈계산
			...
			{
				player[turn]->pae_in_hand=play_hwa_too(atoi(system_command),player[turn],pan,pturn,player); 
				if (hurry_check==0)
					nine_check=check_nine_hwa_too(player[turn]);
				if (nine_check==1)
					hurry_check=1;
			}
			else if (strcmp(system_command,"bomb")==0)
			{
				player[turn]->pae_in_hand=clear_bomb(player[turn]);
				//먹는 함수 구현하	
			}
			else
			{
			...
			for(i=0;i<3;i++)
			{
				player[i]->go_plus = player[i]->point;
				player[i]->point = count_checking_hwa_too(player[i]->pae_eat);
				player[i]->go_plus = player[i]->point - player[i]->go_plus;
			}
			if(player[turn]->point>=3)
			{
				if(player[turn]->go_num!=0)
				{
					if(player[turn]->go_plus>0)
					{
						if ((turn)==0)
						...
						if (strcmp(system_command,"s")==0)
						{
							...
							for(i=0;i<3;i++)
							{
								if(player[i]->win==1)
								{
									result_point=count_checking_hwa_too(player[i]->pae_eat);

									if(player[i]->go_num==1)
										result_point+=1;
									else if(player[i]->go_num==2)
										result_point+=2;
									else if(player[i]->go_num==3)
										result_point*=2;
									else if(player[i]->go_num==4)
										result_point*=4;

									money_[i]=result_point;

									if(player[i]->mungtong==1)
										money_[i]*=2;
									if(player[i]->shake==1)
										money_[i]*=2;//고점수
									a=money_[i]*100;
							...
							for(i=0;i<3;i++)
							{
								if(player[i]->win==0)
								{
									if(player[i]->go_num!=0)
										player[i]->go_bak=1;
									if(player[i]->pi_point<=5)
										player[i]->pi_bak=1;
								}
							}
							...
							for(i=0;i<3;i++)
							{
								...
								if(player[i]->win==0)
								{
									if(player[i]->go_bak==1)
										a*=2;
									if(player[i]->pi_bak==1)
										a*=2;

									money_[i]=a;
									player[i]->money-=money_[i];  //고박 피박 계산
									if(i==0)
										money_[0]=a;
									if(i==1)
										money_[1]=a;
					...
					if (strcmp(system_command,"s")==0)
					{
						...
						for(i=0;i<3;i++){
							if(player[i]->win==1)
							{
								...
								if(find_mungtong_hwa_too(player[i]->pae_eat)>=7)
									money_[i]*=2;
								if(player[i]->shake==1)
									money_[i]*=2;//고점수
								a=money_[i]*100;
						...
						for(i=0;i<3;i++)
						{
							if(player[i]->win==0)
							{
								if(player[i]->go_num!=0)
									player[i]->go_bak=1;
								if(find_pi_hwa_too(player[i]->pae_eat)<=5)
									player[i]->pi_bak=1;
							}
						}

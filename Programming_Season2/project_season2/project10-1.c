...
int result_hwa_too(struct hwa_too_gamer *player[],int *pturn)
{
	...
	for (i=0;i<3;i++)
	{
		if ((i==0) && (player[i]->win==1))
		{
			printf("승자는 A입니다!!!!\n");
		}
		if ((i==1) && (player[i]->win==1))
		{
			printf("승자는 B입니다!!!!\n");
		}
		if ((i==2) && (player[i]->win==1))
		{
			printf("승자는 C입니다!!!!\n");
		}
	}
...
int main(void) {
	...
	int ggo=0,wow,i=0,j=0,command,turn=0,tot=0,sort=0,temp_turn=0,nine_check=0,hurry_check=0,before_turn=0;
	int *pturn=&turn;

...
void play_hwa_too(int count, struct hwa_too_gamer *player, struct hwa_too_pan *pan)
{
	...
	if(eat_count=compare_hwa_too(pae1.num,pan->eat,1))			//깔린것과 낸 것이 같은숫자
	{
		...
		else if (pae1.num==pae2.num && (temp_count=compare_hwa_too(pae3.num,pan->eat,1)))											//뒤집은거랑 먹을거중에 하나 존재, 먹는거는 그대로 먹고
		{
...
int main(void) {
	struct hwa_too init[48]={\
		{1,"광",0,NULL},{1,"오",0,NULL},{1,"피",0,NULL},{1,"피",0,NULL},\
	...
	while (1) {
		...
		for (i=0; i<3; i++) {
			print_turn(turn,i);
			if (i==0)
				printf("A%10s : 패)",player[turn]->status);
			if (i==1)
				printf("B%10s : 패)",player[turn]->status);
			if (i==2)
				printf("C%10s : 패)",player[turn]->status);
			print_hwa_too(player[i]->pae_in_hand);
			printf("\n(점수 : %d) 딴 화투) ",player[turn]->point);

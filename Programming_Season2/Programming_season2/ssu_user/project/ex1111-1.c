...
struct status_hwa_too{
	unsigned shake:1, chong_tong:1, go:3, pi_bak:1, go_bak:1, mung_tung:1;
};
...
int result_hwa_too(struct hwa_too_gamer *player[])
{
	...
	for (i=0;i<3;i++)
	{
		if ((i==0) && (player[i]->go_plus>0))
			printf("승자는 A입니다!!!!\n");
		if ((i==1) && (player[i]->go_plus>0))
			printf("승자는 B입니다!!!!\n");
		if ((i==2) && (player[i]->go_plus>0))
			printf("승자는 C입니다!!!!\n");
	}
	while (1)
	{
...
int main(void) {
	...
	while (1){											//전체적인 while문
		...
		srand(time(NULL));
#if DEBUG
		printf("player[0]->player_status.go=%d\n",player[0]->player_status.go);
		printf("작동\n");
#endif
		for (i=0; i<3; i++)
		{
